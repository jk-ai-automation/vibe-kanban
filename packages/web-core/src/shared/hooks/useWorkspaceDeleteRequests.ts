 ...
 * 工作区删除申请的读写入口。
 *
 * **个人版彻底关掉**：`isLocalPersonalMode()` 为真时查询 `enabled: false`，
 * 一个请求都不会发出去，`getAffordance` 恒返回 `undefined`，
 * 界面退回历史行为。
 *
...
export function useWorkspaceDeleteRequests() {
  ...
  const query = useQuery({
    ...
    refetchInterval: isPersonal ? false : 30_000,
  });
  ...
  const getAffordance = useCallback(
    ...
    [isPersonal, isAdmin, userId, byWorkspaceId]
  );
  ...
  return {
    ...
    requests,
    byWorkspaceId,
    isLoading: query.isPending && !isPersonal,
