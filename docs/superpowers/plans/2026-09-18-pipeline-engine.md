...
12. **变更钩子读到未提交前的旧数据（任务 5 之后追加修复）**：sqlite 的 `update_hook` 在语句执行时、事务提交**之前**触发，`events.rs` 原先每个事件 spawn 一个任务、用另一个连接按 rowid 反查，WAL 下若早于提交就读到旧快照——UPDATE 推出旧值（如 `status` 仍为 `running`）、INSERT 读不到行而丢掉 add，且之后不会补推，前端停在旧状态；preupdate 里同步推的 remove 在事务回滚时也会误推。所有钩子表（workspaces、execution_processes、scratch、issues、project_statuses、issue_comments、pipeline_runs、pipeline_stage_runs）都受影响，流水线状态推送尤其明显。**修复（按连接合并）**：每个连接一份缓冲；`update_hook` 只记 (表, rowid, 插入/更新)，`preupdate_hook` 的删除只记就地生成的 remove 补丁，都不再立即推送；`commit_hook` 触发时整体取走缓冲、spawn 一个任务（回调返回 true，不否决提交）；`rollback_hook` 清空缓冲，回滚掉的写入不推任何补丁。任务先过**提交屏障**：在专用屏障池（同库同参数、1 个连接、懒连接、不装钩子，与业务主池和反查池分开）上 `BEGIN IMMEDIATE`——commit hook 在提交真正完成前触发，要等写事务结束——拿到后**立即回滚释放写锁**，屏障只作「写者已提交」的同步点，反查期间不占写锁，别的写者不必等推送结束；之后在全局一把 tokio Mutex（先来先得）内先推删除，再按 (表, rowid) 去重、保持首次出现顺序反查推送（含 `find_by_id_with_status`、`push_workspace_update_for_session` 这类二次查询）。一个事务只过一次屏障。屏障等待失败（超过 busy_timeout）时打 error 并丢弃这次提交的推送，客户端需重连后才能恢复这些行的状态。**一致性**：同一行最终一致——每次反查都在各自提交之后，推送串行，最后一个推送读到的是所有提交之后的数据；不保证同一次推送内多个读是同一快照（期间别的写者可以提交，读到的只会更新）。**已知边界**：`ROLLBACK TO` 保存点不触发 rollback hook，回滚到保存点的删除仍会推 remove；DELETE 模式下 COMMIT 因 BUSY 失败后再回滚的事务同理。回归测试：`wal_下写事务延迟提交时_replace_补丁带的是提交后的值` / `delete_模式下…`、`wal_下写事务延迟提交时_insert_仍产出_add_补丁` / `delete_模式下…`、`回滚的_insert_不推任何补丁`、`回滚的_delete_不推_remove`、`同一事务多次更新同一行只推一次最终值`、`一个事务多次写只过一次提交屏障`、`推送进行中别的写事务不必等推送结束`。
